...
def non_lin_Wendroff_mod(M, N, x0=0, xf=1, t0=0, tf=1):
    ...
    for n in range(N):
        for m in range(M + 1):
            ...
            if m == 0:
                h[m, n + 1] = h[m, n] - 1 / 2 * dt / dx * (
                va * (h[m + 1, n] - h[m + 1, n]) + ha * (v[m + 1, n] + v[m + 1, n])) + 1 / 2 * (dt / dx) ** 2 * (
                (va ** 2 + ha * g) * (h[m + 1, n] - 2 * h[m, n] + h[m + 1, n]) + 2 * va * ha * (
                v[m + 1, n] - 2 * v[m, n] - v[m + 1, n]))
                v[m, n + 1] = v[m, n] - 1 / 2 * dt / dx * (
                g * (h[m + 1, n] - h[m + 1, n]) + va * (v[m + 1, n] + v[m + 1, n])) + 1 / 2 * (dt / dx) ** 2 * (
                2 * g * va * (h[m + 1, n] - 2 * h[m, n] + h[m + 1, n]) + (g * ha + va ** 2) * (
                v[m + 1, n] - 2 * v[m, n] - v[m + 1, n]))
            elif m == M:
                h[m, n + 1] = h[m, n] - 1 / 2 * dt / dx * (
                va * (h[m - 1, n] - h[m - 1, n]) + ha * (-v[m - 1, n] - v[m - 1, n])) + 1 / 2 * (dt / dx) ** 2 * (
...
def non_lin_LF(M, N, x0=0, xf=1, t0=0, tf=1):
    ...
    for n in range(N):
        for m in range(M + 1):
            if m == 0:
                h[m, n + 1] = 1 / 2 * (h[m + 1, n] + h[m + 1, n]) - 1 / 2 * (
                (h[m + 1, n] * v[m + 1, n]) - (h[m + 1, n] * v[m + 1, n])) * dt / dx
                v[m, n + 1] = 1 / 2 * (v[m + 1, n] - v[m + 1, n]) - 1 / 2 * (
                (g * h[m + 1, n] + 1 / 2 * v[m + 1, n] ** 2) - (g * h[m + 1, n] + 1 / 2 * v[m + 1, n] ** 2)) * dt / dx
            elif m == M:
                h[m, n + 1] = 1 / 2 * (h[m - 1, n] + h[m - 1, n]) - 1 / 2 * (
                (h[m - 1, n] * v[m - 1, n]) - (h[m - 1, n] * v[m - 1, n])) * dt / dx
                v[m, n + 1] = 1 / 2 * (- v[m - 1, n] + v[m - 1, n]) - 1 / 2 * (
